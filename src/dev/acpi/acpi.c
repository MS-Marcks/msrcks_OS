...
void *init_acpi_vz(void *rsdp_address)
{
    ...
    printf("RSDP at: 0x%llx Sig: %s Check: %d OEM: %s rev: %d RSDT Address: 0x%llx\n",
...
void init_acpi()
{
    ...
    printf("MCFG at 0x%11x, SIG: %s LEN: 0x%llx rev %d, check: %d, OEM: %d, OTI: %s, OR:%D CID:%d CREV:%d \n",
           mcfg_header,
           signature,
           mcfg_header->header.length,
           mcfg_header->header.revision,
           mcfg_header->header.checksum,
           oemid,
           oemtableid,
           mcfg_header->header.oem_revision,
